...
#
# BEFORE RUNNING:
#   1. Replace "cis410-saddhamgama-tfstate" with your actual bucket name
...
terraform {
  ...
  #
  # REPLACE: change cis410-saddhamgama-tfstate to your actual bucket name.
  backend "gcs" {
    bucket = "cis410-saddhamgama-tfstate"   # ← your bucket name
...
data "terraform_remote_state" "week7" {
  backend = "gcs"
  config = {
    bucket = "cis410-saddhamgama-tfstate"   # ← same bucket as above
